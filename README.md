...
Thermo RAW access is implemented in `src/main/java/org/searlelab/msrawjava/io/thermo`. `ThermoRawFile.java` is a thin client that talks to a local, self-contained .NET server process managed by `GrpcServerLauncher.java` and pooled by `ThermoServerPool.java`. The client exposes the same high-level methods as the timsTOF reader, including TIC, gradient length, DIA windows, and MS1/MS2 enumeration. Because the server is launched on demand and bound to the current OS and architecture, Java code does not depend on COM or system-installed SDKs; connection details and lifecycle are encapsulated under the `org.searlelab.msrawjava.io.thermo` package. A small gRPC server is used because calling Thermo’s C#/.NET code directly from Java via JNI can be messy and crash-prone, while gRPC lets the C# code run safely in its own process.
