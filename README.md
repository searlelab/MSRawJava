...
**MS**Forest and **MS**RawJava are freely distributed under the [Apache License 2.0](https://www.apache.org/licenses/LICENSE-2.0.txt), but RawFileReader requires accepting the [Thermo RawFileReader license](https://github.com/searlelab/MSRawJava/blob/main/licenses/Thermo-RawFileReader.txt).
...
## Building the code
**Complete build instructions are in `QUICKSTART.md`.** This project targets Java 11 and builds with Maven. Packaging compiles Java and stages the native components (Rust/JNI for Bruker, self-contained .NET gRPC server for Thermo). Dependencies include: JDK 11+, Maven 3.9+, .NET SDK 8.0.x (for the Thermo server), Rust toolchain (rustup, rustc, cargo), and Zig (version≥0.12) and `cargo-zigbuild` for cross-compiling JNI. The software can be built using the command:
