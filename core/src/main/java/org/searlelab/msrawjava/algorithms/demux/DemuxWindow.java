...
public class DemuxWindow implements Comparable<DemuxWindow> {
	 ...
	 *
	 * @param range
	 *            the m/z bounds of this sub-window
	 * @param index
	 *            the column index in the design matrix
	 */
	 ...
	 *
	 * @param windowLower
	 *            the lower bound of the acquired window
	 * @param windowUpper
	 *            the upper bound of the acquired window
	...
	public boolean isContainedBy(double windowLower, double windowUpper) {
		...
		return range.getStart()>=windowLower-BOUNDARY_TOLERANCE&&range.getStop()<=windowUpper+BOUNDARY_TOLERANCE;
	}
	...
	public boolean equals(Object obj) {
		...
		return this.index==other.index&&Double.compare(this.range.getStart(), other.range.getStart())==0
	...
	public String toString() {
		return "DemuxWindow["+index+": "+String.format("%.2f", range.getStart())+"-"+String.format("%.2f", range.getStop())+" Th]";
	}
}
