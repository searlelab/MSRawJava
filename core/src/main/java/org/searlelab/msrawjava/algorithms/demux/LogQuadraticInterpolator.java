...
public class LogQuadraticInterpolator implements RetentionTimeInterpolator {
	...
	public double interpolate(double[] times, double[] intensities, double targetTime) {
		...
		double logResult = quadraticFit(
				times[i0], times[i1], times[i2], times[i3],
				logIntensities[i0], logIntensities[i1], logIntensities[i2], logIntensities[i3],
				targetTime);
	...
	private double quadraticFit(
		...
		double numerator = weight0 * parabolaX0 * (y0 - linearFitAtX0) +
				weight3 * parabolaX3 * (y3 - linearFitAtX3);
		double denominator = weight0 * parabolaX0 * parabolaX0 +
				weight3 * parabolaX3 * parabolaX3;
