...
class StaggeredDemultiplexerUnitTest {
	...
	private static FragmentScan makeScan(String name, int index, double lower, double upper, float rt,
			double[] masses, float[] intensities) {
		return new FragmentScan(
				name, "prec", index, (lower + upper) / 2.0, rt,
				0, 0.0f, lower, upper, masses, intensities,
				null, (byte) 0, lower, upper);
	}
	...
	void testDemultiplexRejectsMismatchedCycleSizes() {
		...
		assertThrows(IllegalArgumentException.class,
				() -> demux.demultiplex(cycleA, cycleA, cycleB, cycleA, cycleA, 1));
	}
	...
	void testDemultiplexRejectsEmptyCenterCycle() {
		...
		assertThrows(IllegalArgumentException.class,
				() -> demux.demultiplex(cycle, cycle, empty, cycle, cycle, 1));
	}
	...
	void testDeduplicatesAnchorTransitionsAndInterpolates() {
		...
		double[][] massesA = new double[][] {
			{500.0000, 500.0004},
			{500.0000}
		};
		float[][] intensitiesA = new float[][] {
			{6.0f, 4.0f},
			{10.0f}
		};

		double[][] massesB = new double[][] {
			{500.0000},
			{500.0000}
		};
		float[][] intensitiesB = new float[][] {
			{10.0f},
			{10.0f}
		};
		...
		ArrayList<FragmentScan> outputs = demux.demultiplex(
				cycleM2, cycleM1, cycleCenter, cycleP1, cycleP2, 1);
