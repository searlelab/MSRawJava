...
class PwizValidationTest {
	...
	void testCompareOriginalVsDemux() throws Exception {
		...
		assertTrue(demuxAvgWidth>origAvgWidth*0.4&&demuxAvgWidth<origAvgWidth*0.6,
				"Demux windows should be approximately half original width (expected 0.4-0.6x, got "+String.format("%.2fx", demuxAvgWidth/origAvgWidth)+")");
	}
	...
	private void compareAndPrint(String peptide, ChromatogramResult orig, ChromatogramResult demux) {
		...
		assertTrue(ratio>0.5&&ratio<2.0, peptide+" demux peak should be within 50%-200% of original");
	}
	...
	void testValidateAgainstPwiz_GTGIVSAPVPK() throws Exception {
		...
		for (int i=0; i<Math.min(20, pwizChrom.times.size()); i++) {
			System.out.printf("  %.2f min: %.1f%n", pwizChrom.times.get(i)/60.0, pwizChrom.intensities.get(i));
		}

		// The key validation would be to run our demux and compare
		// For now, just verify pwiz output is reasonable
		assertTrue(pwizChrom.getMaxIntensity()>100, "pwiz chromatogram should have significant intensity");
	}
	 ...
	 */
	private ChromatogramResult extractChromatogram(EncyclopeDIAFile file, double precursorMz, double[] fragmentMzs) throws Exception {
	...
	void testCosineSimilarityJavaVsPwiz() throws Exception {
		...
		for (int i=2; i<cycles.size()-2; i++) {
			...
			for (FragmentScan javaScan : javaResult) {
				...
				Range subWindowRange=new Range((float)javaScan.getIsolationWindowLower(), (float)javaScan.getIsolationWindowUpper());
				...
				ArrayList<FragmentScan> pwizSpectra=demuxFile.getStripes(subWindowCenter, targetRT-1.0f, targetRT+1.0f, false);

				if (pwizSpectra.isEmpty()) {
					if (showDebug&&debugCount<5) {
						System.out.printf("  No pwiz match for subWindow %.1f-%.1f at RT=%.2f (Java has %d peaks)%n", javaScan.getIsolationWindowLower(),
								javaScan.getIsolationWindowUpper(), targetRT, javaScan.getMassArray().length);
						debugCount++;
					}
					continue;
				}
				...
				if (showDebug&&debugCount<5) {
					System.out.printf("  Match: subWindow %.1f-%.1f, Java %d peaks, pwiz %d peaks at RT=%.2f%n", javaScan.getIsolationWindowLower(),
							javaScan.getIsolationWindowUpper(), javaScan.getMassArray().length, pwizScan.getMassArray().length, targetRT);
					debugCount++;
				}
		...
		for (Map.Entry<Range, List<Double>> entry : perWindowSimilarities.entrySet()) {
			...
			if (sims.size()>=3) {
				...
				if (windowCount<10) {
					System.out.printf("  Window %.1f-%.1f Th: median=%.4f (n=%d)%n", entry.getKey().getStart(), entry.getKey().getStop(), median, sims.size());
				}
				windowCount++;
			}
		}
		...
		if (!cycleMedians.isEmpty()) {
			...
			System.out.printf("\nTime bias check: Early cycles median=%.4f, Late cycles median=%.4f%n", earlyMedian, lateMedian);
		...
		assertTrue(allSimilarities.size()>=50, "Should have at least 50 comparisons for statistical validity, got "+allSimilarities.size());
		System.out.println("✓ Sufficient comparisons: "+allSimilarities.size());

		// The 95th percentile should show some good matches exist
		assertTrue(percentiles[4]>0.80, "95th percentile should show some good matches (> 0.80), got "+String.format("%.4f", percentiles[4]));
		System.out.println("✓ 95th percentile > 0.80 (best matches are good)");

		// The median should be positive (some correlation exists)
		assertTrue(percentiles[2]>0.0, "Median cosine similarity should be positive, got "+String.format("%.4f", percentiles[2]));
		...
		System.out.printf("  5th percentile:  %.4f (target: > 0.80)%n", percentiles[0]);
		System.out.printf("  25th percentile: %.4f (target: > 0.95)%n", percentiles[1]);
		System.out.printf("  Median:          %.4f (target: > 0.95)%n", percentiles[2]);
		System.out.printf("  75th percentile: %.4f (target: > 0.99)%n", percentiles[3]);
		System.out.printf("  95th percentile: %.4f (target: > 0.999)%n", percentiles[4]);

		float[] percents=new float[] {0.05f, 0.25f, 0.5f, 0.75f, 0.95f};
		float[] targets=new float[] {0.8f, 0.95f, 0.95f, 0.99f, 0.999f};
		for (int i=0; i<targets.length; i++) {
			assertTrue(percentiles[i]>targets[i], Math.round(percents[i]*100f)+"th percentile should be better than "+targets[i]+", but got "+percentiles[i]);
		}
	...
	void testDiagnoseEmptyScansAndCycles() throws Exception {
		...
		for (int i=0; i<cycles.size(); i++) {
			...
			System.out.printf("  Cycle %d: %d spectra, RT=%.2f-%.2f sec%n", i, cycle.spectra.size(), cycle.startRT, cycle.getCenterRT());
		}
		...
		for (int i=2; i<cycles.size()-2; i++) {
			...
			System.out.printf("  Cycle sizes: %d, %d, %d, %d, %d%n", cycleM2.spectra.size(), cycleM1.spectra.size(), cycleCenter.spectra.size(),
			...
			boolean allSameSize=cycleM2.spectra.size()==expectedSize&&cycleM1.spectra.size()==expectedSize&&cycleP1.spectra.size()==expectedSize
			...
			for (FragmentScan scan : result) {
				if (scan.getMassArray().length==0) {
					...
					boolean isLastEdge=Math.abs(scan.getIsolationWindowLower()-lastEdgeLower)<0.5&&Math.abs(scan.getIsolationWindowUpper()-lastEdgeUpper)<0.5;
			...
			if (!nonEdgeEmptyScans.isEmpty()) {
				...
				for (FragmentScan s : nonEdgeEmptyScans) {
					System.out.printf("    %.2f-%.2f Th at RT=%.3f min%n", s.getIsolationWindowLower(), s.getIsolationWindowUpper(), s.getScanStartTime()/60f);
				}
			}
			assertTrue(nonEdgeEmptyScans.isEmpty(),
					"Empty demux scans should only occur for edge sub-windows. Found "+nonEdgeEmptyScans.size()+" non-edge empty scans.");
		...
		System.out.println("\n=== Summary ===");
		System.out.printf("Total cycles processed: %d (of %d available, dropped %d at start, %d at end)%n", Math.max(0, cycles.size()-4), cycles.size(), 2, 2);
	...
	void testTraceSubWindowProcessing() throws Exception {
		...
		for (int i=0; i<Math.min(10, cycleCenter.spectra.size()); i++) {
			...
			System.out.printf("  %d: %.2f-%.2f Th (%.2f wide), RT=%.2f, %d peaks%n", i, scan.getIsolationWindowLower(), scan.getIsolationWindowUpper(),
					scan.getIsolationWindowUpper()-scan.getIsolationWindowLower(), scan.getScanStartTime(), scan.getMassArray().length);
		}
	...
	private List<DemuxCycle> extractCycles(EncyclopeDIAFile file, ArrayList<Range> windows) throws Exception {
		...
		if (!cycles.isEmpty()) {
			System.out.println("  First cycle: "+cycles.get(0).spectra.size()+" spectra, RT="+String.format("%.2f", cycles.get(0).startRT)+" sec");
