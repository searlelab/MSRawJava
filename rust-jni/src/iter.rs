...
impl RtIterator {
    pub fn new(ds: std::sync::Arc<Dataset>, indices: Vec<usize>,
               mz_lo: f64, mz_hi: f64, scan_lo: i32, scan_hi: i32) -> Self {
        let mut with_rt: Vec<(usize, f64)> = indices.into_iter()
    ...
    pub fn next(&mut self) -> Result<Option<SpectrumArrays>, String> {
        while self.i < self.frames_sorted_by_rt.len() {
			...
			let ms_level = match frame.ms_level {
			    MSLevel::MS1 => 1u32,
			    MSLevel::MS2 => 2u32,
			    MSLevel::Unknown => 0u32,  // allowed; Java picked frames
			};
            ...
            if !(self.mz_lo.is_finite() && self.mz_hi.is_finite() && self.mz_lo < self.mz_hi) {
                return Err("Invalid m/z window".to_string());
            }
            let mzc = &self.ds.meta.mz_converter;
            let mut tof_lo = mzc.invert(self.mz_lo).floor();
            let mut tof_hi = mzc.invert(self.mz_hi).ceil();
            ...
            let imc = &self.ds.meta.im_converter;
			...
			return Ok(Some(SpectrumArrays {
			    ...
			    frame_index: frame_idx as u32, rt_seconds
			}));
        }
        Ok(None)
    }
